 ...
 */
#ifdef WOLFSSL_DEMO_EX
#include "wolfssl_example_ex.h"
#include "picture_encode_data_large.h"
#include "picture_encode_data_middle.h"
#include "picture_encode_data_small.h"
...
static int tls13_uart_client_picture_upload(const char encoded_picture_data[])
{
    int ret = -1, err;
    WOLFSSL_CTX* ctx = NULL;
    WOLFSSL* ssl = NULL;
//    const char testStr[] = "Testing 1, 2 and 3\r\n";
    ...
    do {
        ret = wolfSSL_write(ssl, encoded_picture_data, XSTRLEN(encoded_picture_data));
        err = wolfSSL_get_error(ssl, ret);
    } while (err == WOLFSSL_ERROR_WANT_READ || err == WOLFSSL_ERROR_WANT_WRITE);
//    printf("Sent (%d): %s\n", err, encoded_picture_data);
...
//    printf("Read (%d): %s\n", err, readBuf);
	printf("Picture data sent complete!\n");
    ret = 0; /* Success */
...
#ifdef CMSIS_OS2_H_
void wolfCryptDemo_ex(void* argument)
#else
void wolfCryptDemo_ex(const void* argument)
#endif
{
	...
	while (1) {
        ...
        do {
    		halRet = HAL_UART_Receive(&HAL_CONSOLE_UART, buffer, sizeof(buffer), 1000);
        ...
        case 'l':
            ...
            printf("Large Picture Upload Server...\n");
            args.return_code = tls13_uart_client_picture_upload(picture_encode_data_large);
        #else
//            args.return_code = NOT_COMPILED_IN;
        ...
        case 'm':
        #if !defined(NO_WOLFSSL_CLIENT)
            printf("Middle Picture Upload Server...\n");
            args.return_code = tls13_uart_client_picture_upload(picture_encode_data_middle);
        #else
//            args.return_code = NOT_COMPILED_IN;
        #endif
            printf("TLS 1.3 Client: Return code %d\n", args.return_code);
        ...
        case 's':
        #if !defined(NO_WOLFSSL_CLIENT)
            printf("Small Picture Upload Server...\n");
            args.return_code = tls13_uart_client_picture_upload(picture_encode_data_small);
        #else
//            args.return_code = NOT_COMPILED_IN;
        #endif
            printf("TLS 1.3 Client: Return code %d\n", args.return_code);
